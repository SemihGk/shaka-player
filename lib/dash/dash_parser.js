...
shaka.dash.DashParser.prototype.processManifest_ =
  ...
  let periodsAndDuration = await this.parsePeriods_(context, baseUris, mpd);
 ...
 * @private
 */
shaka.dash.DashParser.prototype.parsePeriods_ = async function(
  ...
  for (let i = 0; i < periodNodes.length; i++) {
    ...
    if (this.periodIds_.indexOf(periodId) == -1) {
      ...
      if (this.manifest_) {
        await this.playerInterface_.updateSupportedStreams(period);
