  ...
  return elems.map(
      ...
      function(elem) {
        ...
        if (!psshs.length && ContentProtection.defaultKeySystems_[schemeUri] ===
          ...
          var pssh = shaka.dash.ContentProtection
...
shaka.dash.ContentProtection.buildPssh = function(schemeUri, dataBase64) {
  ...
  var adjustedData = shaka.dash.ContentProtection.addCustomProperty_(data);
...
shaka.dash.ContentProtection.addCustomProperty_ = function(data) {
  var checkSumArray = [60, 0, 67, 0, 72, 0, 69, 0, 67, 0, 75, 0, 83, 0, 85, 0,
    77, 0, 62];
  var checksum = new Uint8Array(checkSumArray); // <CHECKSUM>
  var checksumLength = checksum.length;
  var laUrl =
      '<LA_URL>' +
          'http://api.cld.dtvce.com/rights/management/' +
          'mdrm/vgemultidrm/v1/playready/license' +
      '</LA_URL>';
  var laUrlArray = new TextEncoder('utf-8').encode(laUrl);

  var index = -1;
  data.forEach(function(byte, ind) {
    var selectedArray = data.slice(ind, checksumLength + ind);
  ...
  var msProLength = (laUrlArray.length * 2) + data.length;

  var msProBuffer = new ArrayBuffer(msProLength);
  var writer = new shaka.util.DataViewWriter(
      new DataView(msProBuffer),
      shaka.util.DataViewWriter.Endianness.BIG_ENDIAN);

  writer.writeUint32(msProLength, true);
  var playreadyBlockType = data.slice(4, 8);
  writer.put(playreadyBlockType);
  writer.writeUint16(msProLength - 10, true);

  var firstPart = data.slice(10, index);
  var lastPart = data.slice(index);
