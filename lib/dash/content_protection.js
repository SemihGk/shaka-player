  ...
  return elems.map(
      ...
      function(elem) {
        ...
        if (!psshs.length && ContentProtection.defaultKeySystems_[schemeUri] ===
          ...
          let msProNode = shaka.util.XmlUtils.findChild(elem, 'mspr:pro');
          let msProText = msProNode.textContent;
          let pssh = shaka.dash.ContentProtection
              .buildPssh(schemeUri, msProText);
          psshs.push(pssh);
        }
...
shaka.dash.ContentProtection.buildPssh = function(schemeUri, dataBase64) {
  let data = shaka.util.Uint8ArrayUtils.fromBase64(dataBase64);
  let adjustedData = shaka.dash.ContentProtection.addCustomProperty_(data);

  let schemeUriHexStr = schemeUri.match(/[\da-f]{2}/gi).join('');
  let schemeUriData = shaka.util.Uint8ArrayUtils.fromHex(schemeUriHexStr);

  let psshBoxData = new Uint8Array(shaka.util.StringUtils.toUTF8('pssh'));

  let psshBoxLength = 12 /* Full Header Size */ + 16 /* schemeUri */ +
      4 /* DataSize */ + adjustedData.length;

  let psshArrayBuffer = new ArrayBuffer(psshBoxLength);
  let writer = new shaka.util.DataViewWriter(
  ...
  writer.writeUint32(adjustedData.length);
  writer.put(adjustedData);

  let psshByteArray = writer.getArray();
...
/**
 * Add custom property to ms:pro element.
 *
 * @param {Uint8Array} data
 * @return {Uint8Array}
 * @private
 */
shaka.dash.ContentProtection.addCustomProperty_ = function(data) {
  let checkSumArray = [60, 0, 67, 0, 72, 0, 69, 0, 67, 0, 75, 0, 83, 0, 85, 0,
    77, 0, 62];
  let checksum = new Uint8Array(checkSumArray); // <CHECKSUM>
  let checksumLength = checksum.length;
  let laUrl =
      '<LA_URL>' +
          'https://api.cld.dtvce.com/rights/management/' +
          'mdrm/vgemultidrm/v1/playready/license' +
      '</LA_URL>';
  let laUrlArray = new TextEncoder('utf-8').encode(laUrl);

  let index = -1;
  data.forEach(function(byte, ind) {
    let selectedArray = data.slice(ind, checksumLength + ind);
    if (selectedArray.join('') == checksum.join('')) {
      index = ind;
    }
  });
  ...
  let msProLength = (laUrlArray.length * 2) + data.length;

  let msProBuffer = new ArrayBuffer(msProLength);
  let writer = new shaka.util.DataViewWriter(
      new DataView(msProBuffer),
      shaka.util.DataViewWriter.Endianness.BIG_ENDIAN);

  writer.writeUint32(msProLength, true);
  let playreadyBlockType = data.slice(4, 8);
  writer.put(playreadyBlockType);
  writer.writeUint16(msProLength - 10, true);

  let firstPart = data.slice(10, index);
  let lastPart = data.slice(index);
  writer.put(firstPart);
  laUrlArray.forEach(function(byte) {
