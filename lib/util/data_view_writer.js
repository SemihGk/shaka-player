 ...
 * @export
 */
shaka.util.DataViewWriter.prototype.changePositions = function(array, firstArray, secondArray) {
  try {
    ...
    var firstMatchIndex = -1; // not found yet.
    function isMatch(index, array, matchArray) {
    ...
    array.forEach(function(item, index, array) {
      if (isMatch(index, array, firstArray)) {
    ...
    for (var i = 0; i < array.length; i++) {
      if (i < firstMatchIndex || i >= (firstMatchIndex + firstArray.length + secondArray.length)) {
        this.writeUint8(array[i]);
      }
