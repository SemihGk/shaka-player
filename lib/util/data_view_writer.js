...
shaka.util.DataViewWriter.prototype.isMatch =
    function(index, array, matchArray) {
  let matchString = matchArray.join('');
  let arrayString = array
      .slice(index, matchArray.length + index).join('');

  return matchString === arrayString;
};
 ...
 * @export
 */
shaka.util.DataViewWriter.prototype.changePositions =
    function(array, firstArray, secondArray) {
  try {
    if (firstArray.length < 2 ||
        array.length <= firstArray ||
        array.length <= secondArray) {
      return;
    }

    let firstMatchIndex = -1; // not found yet.


    array.forEach(function(item, index, array) {
      if (this.isMatch(index, array, firstArray)) {
    ...
    for (let i = 0; i < array.length; i++) {
      if (i < firstMatchIndex || i >=
        (firstMatchIndex + firstArray.length + secondArray.length)) {
        this.writeUint8(array[i]);
      }

      if (i === firstMatchIndex) {
        this.put(secondArray);
      }

      if (i === (firstMatchIndex + secondArray.length)) {
        this.put(firstArray);
      }
    }
  } catch (exception) {
