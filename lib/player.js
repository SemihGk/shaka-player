...
shaka.Player.prototype.load = async function(manifestUri, opt_startTime,
  ...
  try {
    ...
    // if it is live and yospace manifest exist, we have to force player to
    // start with encrypted content.
    // If it is initial manifest clear, don't do anything.
    // We have to discuss with yospace team to add protected blocks to them
    // ad manifests.

    if (this.isLive()) {
      let contentResult = this.hasClearContent_(opt_startTime);
      opt_startTime = contentResult.startTime;

      // On Tizen, clockSync issue is a problem more often than web.
      // Current workaround is that we set offset the difference between
      // client time and manifest availability time.
      let presentationStartTime =
          this.manifest_.presentationTimeline.getPresentationStartTime();
      let offsetTimeline = ((Date.now() / 1000) -
          presentationStartTime) / 1000;
      this.manifest_.presentationTimeline.setClockOffset(offsetTimeline);

      if (contentResult.clearManifest) {
        return this
            .load(manifestUri, opt_startTime, opt_manifestParserFactory);
      }
    }
...
/**
 * Detect clear content starting manifst for Tizen Live DAI bug.
 * @param {?number=} opt_startTime
 * @return {Object}
 * @private
 */
shaka.Player.prototype.hasClearContent_ = function(opt_startTime) {
  let manifest = this.manifest_;
  let isInitialPeriodClear = false;
  let clearManifest = false;
  let neededStartTime = opt_startTime;
  let encryptionMaps = [];
  manifest.periods.some(function(period, index) {
    let isYospacePeriod = period.variants.every(function(variant) {
      return !variant.drmInfos.length;
    });

    encryptionMaps.push({
      index: index,
      isClear: isYospacePeriod
    });

    if (!index && isYospacePeriod) {
      isInitialPeriodClear = true;
    }

    if (!neededStartTime && !isInitialPeriodClear && isYospacePeriod) {
      let diff = manifest.periods[index].startTime -
          manifest.periods[index - 1].startTime;

      neededStartTime = diff > 6 ?
          (manifest.periods[index].startTime - 6) :
          manifest.periods[index - 1].startTime;
    }

    return isYospacePeriod;
  });

  if (isInitialPeriodClear) {
    let encryptedItem = null;
    encryptionMaps.forEach(function(item) {
      if (!item.isClear) {
        encryptedItem = item;
      }
    });
    ...
    if (encryptedItem) {
      neededStartTime = manifest.periods[encryptedItem.index].startTime;
    } else {
      clearManifest = true;
    }
  }
  ...
  return {
    startTime: neededStartTime,
    clearManifest: clearManifest
  };
};
