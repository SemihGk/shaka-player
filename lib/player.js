...
shaka.Player.prototype.load = function(manifestUri, opt_startTime,
  ...
  return loadChain.then(function() {
    ...
    if (this.isLive()) {
      var contentResult = this.hasClearContent_(opt_startTime);
...
shaka.Player.prototype.hasClearContent_ = function(opt_startTime) {
  ...
  var isInitialPeriodClear = false;
  var clearManifest = false;
  var neededStartTime = opt_startTime;
  var encryptionMaps = [];
  manifest.periods.some(function(period, index) {
    var isYospacePeriod = period.variants.every(function(variant) {
    ...
    if (!neededStartTime && !isInitialPeriodClear && isYospacePeriod) {
      var diff = manifest.periods[index].startTime -
