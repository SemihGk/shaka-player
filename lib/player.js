...
shaka.Player.prototype.load = function(manifestUri, opt_startTime,
  ...
  return loadChain.then(function() {
    ...
    var playerInterface = {
      ...
      checkDrmInitialization: this.checkDrmInitialization_.bind(this),
...
shaka.Player.prototype.checkDrmInitialization_ = function(manifest, periods) {
  var drmEngine = this.drmEngine_;
  var isDrmEngineActive = !!drmEngine && !!drmEngine.currentDrmInfo_;

  // Drm Engine Active, so no need any check;
  if (!isDrmEngineActive) {
    var isEncryptedManifestUpdate = periods.some(function(period) {
      period.variants.some(function(variant) {
        return !!variant.drmInfos.length;
      });
    });

    if (isEncryptedManifestUpdate) {
      return drmEngine.init(manifest, false /* offline */)
        .then(function() {
          return drmEngine.attach(this.video);
        }.bind(this));
        // TODO then re-filter last periods since drmEngine activated
    }
  }
  // Nothing else to check, go ahead!
  return Promise.resolve();
}
