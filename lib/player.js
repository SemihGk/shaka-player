...
shaka.Player.prototype.load = function(manifestUri, opt_startTime,
  ...
  return loadChain.then(function() {
    ...
    function hasClearContent(manifest) {
      let isInitialPeriodClear = false;
      let neededStartTime = null;
      let encryptionMaps = [];
      let hasClearContent = manifest.periods.some(function(period, index) {
      ...
      if (hasClearContent) {
        opt_startTime = neededStartTime;
        const offsetTimeline = ((Date.now() / 1000) -
            _this.manifest_.presentationTimeline.presentationStartTimer)/ 1000;
        _this.manifest_.presentationTimeline.setClockOffset(offsetTimeline);
      }
    }

    if (this.isLive()) {
      hasClearContent(this.manifest_);
      if (clearManifest) {
        return _this
...
shaka.Player.prototype.filterAllPeriods_ = function(periods) {
  ...
  let validPeriodsCount = ArrayUtils.count(periods, function(period) {
    if (!period.variants.some(StreamUtils.isPlayable)) {
...
shaka.Player.prototype.filterNewPeriod_ = function(period) {
  ...
  if (!hasPlayableVariant) {
    console.log('playable 2', hasAvailableVariant,
    hasPlayableVariant, variants, period);
    throw new shaka.util.Error(
        shaka.util.Error.Severity.CRITICAL,
        shaka.util.Error.Category.MANIFEST,
        shaka.util.Error.Code.UNPLAYABLE_PERIOD);
  ...
  if (keyIds.length) {
    period.variants.forEach(function(variant) {
      ...
      streams.forEach(function(stream) {
        ...
        //   console.log(keyStatus, stream.keyId, keyStatusMap);
