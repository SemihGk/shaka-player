...
shaka.Player.prototype.loadManifest_ = function() {
  ...
  let playerInterface = {
    ...
    updateSupportedStreams: this.updateSupportedStreams.bind(this),
...
/**
 * Takes a period and updates supportedTypes of drmEngine.
 *
 * @param {shaka.extern.Period} period
 * @return {!Promise}
 * @private
 */
shaka.Player.prototype.updateSupportedStreams =
    async function(period) {
  if (!this.drmEngine_) {
    return;
  }

  const codecProfiles = period.variants.reduce((codecProfiles, variant) => {
      const audioMimeType = shaka.util.MimeUtils
          .getFullType(variant.audio.mimeType, variant.audio.codecs);
      const videoMimeType = shaka.util.MimeUtils
          .getFullType(variant.video.mimeType, variant.video.codecs);

      if (codecProfiles.indexOf(audioMimeType) === -1) {
        codecProfiles.push(audioMimeType);
      }

      if (codecProfiles.indexOf(videoMimeType) === -1) {
        codecProfiles.push(videoMimeType);
      }
  ...
  const supportedStreams =
      this.drmEngine_.getSupportedStreams();

  const isExistedProfile = codecProfiles.every((codecProfile) =>
  supportedStreams.indexOf(codecProfile) !== -1);

  if (isExistedProfile) {
    return;
  }

  await this.drmEngine_.updateSupportedStreams(codecProfiles);
};
  ...
  if (keyIds.length) {
    period.variants.forEach(function(variant) {
      ...
      streams.forEach(function(stream) {
        ...
        if (stream.keyId && navigator.userAgent.indexOf('Tizen') === -1) {
