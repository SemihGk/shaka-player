...
shaka.Player.prototype.load = async function(
  ...
  try {
    ...
    if (this.isLive()) {
      let contentResult = this.hasClearContent_(startTime);
      startTime = contentResult.startTime;

      const offsetTimeline = ((Date.now() / 1000) -
      this.manifest_.presentationTimeline.presentationStartTimer) / 1000;
      this.manifest_.presentationTimeline.setClockOffset(offsetTimeline);

      if (contentResult.clearManifest) {
        return this
            .load(manifestUri, startTime, manifestParserFactory);
      }
    }
 ...
 * @private
 */
shaka.Player.prototype.hasClearContent_ = function(startTime) {
  let manifest = this.manifest_;
  let isInitialPeriodClear = false;
  let clearManifest = false;
  let neededStartTime = startTime;
  let encryptionMaps = [];
