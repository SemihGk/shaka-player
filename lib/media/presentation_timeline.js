...
shaka.media.PresentationTimeline = function(
  ...
  /** @public {?number} */
  this.presentationStartTimer = presentationStartTime;
...
shaka.media.PresentationTimeline.prototype.getPresentationStartTime =
    function() {
  return this.presentationStartTimer;
};
...
shaka.media.PresentationTimeline.prototype.getSafeSeekRangeStart = function(
  ...
  if (this.segmentAvailabilityDuration_) {
    return seekStart;
  }
  ...
  console.log('desiredStart', desiredStart, seekStart, availabilityStart,
    availabilityEnd);
...
shaka.media.PresentationTimeline.prototype.getLiveEdge_ = function() {
  goog.asserts.assert(this.presentationStartTimer != null,
                      'Cannot compute timeline live edge without start time');
  let now = (Date.now() + this.clockOffset_) / 1000.0;
  return Math.max(
      0, now - this.maxSegmentDuration_ - this.presentationStartTimer);
};
