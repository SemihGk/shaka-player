...
shaka.media.DrmEngine.prototype.attach = function(video) {
  ...
  console.log('mediaKeys_', this.mediaKeys_)
...
shaka.media.DrmEngine.prototype.queryMediaKeys_ =
  ...
  p = p.then(function(mediaKeySystemAccess) {
    ...
    console.log(configsByKeySystem);
    var originalConfig = configsByKeySystem[mediaKeySystemAccess.keySystem];
    var passiveDrm = keySystemsInOrder.filter(function(item) {
      return item !== mediaKeySystemAccess.keySystem;
    });
    var passiveConfig = configsByKeySystem[passiveDrm];
    this.createCurrentDrmInfo_(
        mediaKeySystemAccess.keySystem, originalConfig,
        originalConfig.drmInfos, false);
    this.createCurrentDrmInfo_(
        passiveDrm, passiveConfig,
        passiveConfig.drmInfos, true);
...
shaka.media.DrmEngine.prototype.createCurrentDrmInfo_ = function(
    keySystem, config, drmInfos, isPassiveDrm) {
  /** @type {!Array.<string>} */
  var licenseServers = [];
  ...
  var videoRobustness =
      ...
      console.log(initDatas[0], new TextDecoder('utf-8').decode(initDatas[0].initData));
  if (isPassiveDrm) {
    this.currentDrmInfo_.passiveInitData = initDatas;
    return;
  }
...
shaka.media.DrmEngine.prototype.onEncrypted_ = function(event) {
  ...
  console.log('onEncrypted_', event);
...
shaka.media.DrmEngine.prototype.createTemporarySession_ =
  ...
  console.log(session, new TextDecoder('utf-8').decode(initData));
  this.eventManager_.listen(session, 'message',
      /** @type {shaka.util.EventManager.ListenerType} */(
          this.onSessionMessage_.bind(this)));
  this.eventManager_.listen(session, 'keystatuseschange',
