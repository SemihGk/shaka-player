...
shaka.media.DrmEngine = function(playerInterface) {
  ...
  /** @private {!Object.<string, MediaKeySystemConfiguration>} */
  this.configsByKeySystem_ = {};
...
shaka.media.DrmEngine.prototype.init_ = function(variants) {
  ...
  for (const variant of variants) {
    this.prepareMediaKeyConfigsForVariant_(
        variant, this.configsByKeySystem_, keySystemsInOrder);
  }
  ...
  const p = this.queryMediaKeys_(this.configsByKeySystem_, keySystemsInOrder);
...
/**
 * @param {!Array<string>} codecProfiles
 * @return {!Promise} Resolved when CDM query responds.
 */
shaka.media.DrmEngine.prototype.updateSupportedStreams =
    async function(codecProfiles) {
  const keySystem = this.keySystem();
  let config = this.configsByKeySystem_[keySystem];

  for (const codecProfile of codecProfiles) {
    if (config.audioCapabilities && /audio/g.test(codecProfile)) {
      config.audioCapabilities.push({
        robustness: '', /* temporarily. just for test */
        contentType: codecProfile,
      });
    }

    if (config.videoCapabilities && /video/g.test(codecProfile)) {
      config.videoCapabilities.push({
        robustness: '', /* temporarily. just for test */
        contentType: codecProfile,
      });
    }
  ...
  await navigator.requestMediaKeySystemAccess(keySystem, [config])
    .then((mediaKeySystemAccess) => {
      ...
      for (const cap of audioCaps) {
        if (this.supportedTypes_.indexOf(cap.contentType) === -1) {
          this.supportedTypes_.push(cap.contentType);
        }
      }
      for (const cap of videoCaps) {
        if (this.supportedTypes_.indexOf(cap.contentType) === -1) {
...
/**
 * @return {!Array.<string>} supportedTypes
 */
shaka.media.DrmEngine.prototype.getSupportedStreams = function() {
  return this.supportedTypes_ || [];
};
