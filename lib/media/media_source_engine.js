...
shaka.media.MediaSourceEngine.prototype.enqueueOperation_ =
  ...
  if (!document.body.contains(this.video_)) {
      this.stallOperation_.bind(this, contentType, start)
      return operation.p;
  }
...
/**
 * On Tizen, we recieve 3016 errors if we manipulate video element during it is
 * not attached in DOM. So, we need to stall the operations until video element
 * is attached to DOM again.
 *
 * @param {shaka.util.ManifestParserUtils.ContentType} contentType
 * @param {function()} start
 * @return {!Promise}
 * @private
 */
shaka.media.MediaSourceEngine.prototype.stallOperation_ =
    function(contentType, start) {

    setTimeout(function() {
        return this.enqueueOperation_.bind(this, contentType, start);
    }.bind(this), 200);
};
