 ...
 */
shaka.media.StreamingEngine = function(manifest, playerInterface, video) {
...
shaka.media.StreamingEngine.prototype.fetch_ = function(reference) {
  ...
  if (request.uris[0].indexOf('.mp4') !== -1) {
    var event = new CustomEvent('yospacebuffer', {
      detail: {
        url: request.uris[0],
    ...
    this.video_.dispatchEvent(event);
