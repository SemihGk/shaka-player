...
shaka.media.StreamingEngine.prototype.initSourceBuffer_ = function(
  ...
  var appendInit = fetchInit.then(function(initSegment) {
    ...
    // Remove passive drm from init segment
    var initSegmentArray = new Uint8Array(initSegment);
    console.log(initSegment);
    var activeDrmInitData = this.playerInterface_.drmEngine
        .currentDrmInfo_.initData[0].initData; // playready
    var passiveDrmInitData = this.playerInterface_.drmEngine
        .currentDrmInfo_.passiveInitData[0].initData; // widevine

    var adjustedSegmentBuffer = new ArrayBuffer(initSegmentArray.length);
    // var writer = new shaka.util.DataViewWriter(
    //   new DataView(adjustedSegmentBuffer),
    //   shaka.util.DataViewWriter.Endianness.BIG_ENDIAN);
    // console.log(activeDrmInitData.length, passiveDrmInitData.length)
    //
    // writer.changePositions(initSegmentArray, passiveDrmInitData, activeDrmInitData);
    // console.log(initSegmentArray.length, passiveDrmInitData.length, activeDrmInitData.length,
    //   writer.getArray().length);


    console.log(new TextDecoder('utf-8').decode(new Uint8Array(initSegment)));
    console.log(new TextDecoder('utf-8').decode(activeDrmInitData));
    console.log(new TextDecoder('utf-8').decode(passiveDrmInitData));
    console.log(new TextDecoder('utf-8').decode(writer.getArray()));
